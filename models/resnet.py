...
def ResNet(inputs, architecture = 'resnet50', stage5 = False, train_bn = True, name = 'ResNet'):
    """
    ...
    outputs = BatchNormalization(name = 'bn_conv1')(outputs, training = train_bn)
    outputs = ReLU()(outputs)
    C1 = outputs = MaxPool2D(pool_size = (3, 3), strides = 2, padding = 'same')(outputs)
    # Stage 2
    outputs = _conv_block(outputs, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), train_bn=train_bn)
    outputs = _identity_block(outputs, 3, [64, 64, 256], stage=2, block='b', train_bn=train_bn)
    C2 = outputs = _identity_block(outputs, 3, [64, 64, 256], stage=2, block='c', train_bn=train_bn)
    # Stage 3
    outputs = _conv_block(outputs, 3, [128, 128, 512], stage=3, block='a', train_bn=train_bn)
    outputs = _identity_block(outputs, 3, [128, 128, 512], stage=3, block='b', train_bn=train_bn)
    outputs = _identity_block(outputs, 3, [128, 128, 512], stage=3, block='c', train_bn=train_bn)
    C3 = outputs = _identity_block(outputs, 3, [128, 128, 512], stage=3, block='d', train_bn=train_bn)
    # Stage 4
    outputs = _conv_block(outputs, 3, [256, 256, 1024], stage=4, block='a', train_bn=train_bn)
    block_count = {"resnet50": 5, "resnet101": 22}[architecture]
    for i in range(block_count):
        outputs = _identity_block(outputs, 3, [256, 256, 1024], stage=4, block=chr(98 + i), train_bn=train_bn)
    C4 = outputs
    # Stage 5
    if stage5:
        outputs = _conv_block(outputs, 3, [512, 512, 2048], stage=5, block='a', train_bn=train_bn)
        outputs = _identity_block(outputs, 3, [512, 512, 2048], stage=5, block='b', train_bn=train_bn)
        C5 = outputs = _identity_block(outputs, 3, [512, 512, 2048], stage=5, block='c', train_bn=train_bn)
    else:
        C5 = None
    return tf.keras.Model(inputs = inputs, outputs = [C1, C2, C3, C4, C5] if stage5 else C4)
